      ...
      <div className="grid md:grid-cols-12 gap-12 items-center relative z-10">
        <div className="md:col-span-8">
          ...
          <div ref={textRef} className="space-y-8">
            ...
            <p className="text-2xl md:text-4xl font-serif italic text-zinc-300 reveal-text transition-delay-200">
              {t.philosophy.text2Highlight}
            </p>
            <div className="w-24 h-[1px] bg-neon-acid reveal-text transition-delay-300"></div>
            <p className="text-zinc-400 text-lg md:text-xl max-w-2xl font-sans reveal-text transition-delay-400 leading-relaxed">
              {t.philosophy.desc}
            </p>
          </div>
        </div>

        <div className="md:col-span-4 flex flex-col justify-end items-end gap-12 border-l border-white/20 pl-8 md:pl-0 md:border-l-0">
          {t.philosophy.stats.map((stat, idx) => (
            <div key={idx} className={`reveal-text transition-delay-${500 + (idx * 100)} text-right w-full`}>
              ...
              <span className="text-zinc-400 font-mono text-xs uppercase tracking-widest border-t border-zinc-800 pt-2 inline-block w-full text-right">{stat.label}</span>
            </div>
          ))}
        </div>
      </div>
