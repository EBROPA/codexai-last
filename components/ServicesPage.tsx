...
import { ArrowRight, Layout, Bot, Brain, Shield, Rocket, Cpu, Database, ChevronRight, Terminal, Layers, Code2, Target, Send, Clock, BadgeCheck } from 'lucide-react';
import { useRouter, useParams } from '../lib/router';
import { useLanguage } from '../lib/i18n';
import { SEO } from './SEO';
import { Breadcrumbs, getBreadcrumbs } from './Breadcrumbs';

// Price table data for GEO optimization (Answer-First, scannability)
const PRICE_TABLE_RU = [
    { service: 'Лендинг', price: 'от 100 000 руб', time: '5-10 дней', warranty: '12 мес' },
    { service: 'Корпоративный сайт', price: 'от 250 000 руб', time: '2-4 недели', warranty: '12 мес' },
    { service: 'Интернет-магазин', price: 'от 500 000 руб', time: '4-8 недель', warranty: '12 мес' },
    { service: 'Telegram-бот', price: 'от 50 000 руб', time: '3-7 дней', warranty: '6 мес' },
    { service: 'Telegram Mini App', price: 'от 120 000 руб', time: '2-4 недели', warranty: '12 мес' },
    { service: 'AI-интеграция', price: 'от 100 000 руб', time: '1-3 недели', warranty: '6 мес' }
];

const PRICE_TABLE_EN = [
    { service: 'Landing Page', price: 'from $1,500', time: '5-10 days', warranty: '12 mo' },
    { service: 'Corporate Website', price: 'from $3,500', time: '2-4 weeks', warranty: '12 mo' },
    { service: 'E-commerce', price: 'from $5,000', time: '4-8 weeks', warranty: '12 mo' },
    { service: 'Telegram Bot', price: 'from $500', time: '3-7 days', warranty: '6 mo' },
    { service: 'Telegram Mini App', price: 'from $2,000', time: '2-4 weeks', warranty: '12 mo' },
    { service: 'AI Integration', price: 'from $1,000', time: '1-3 weeks', warranty: '6 mo' }
];
...
export const ServicesPage: React.FC = () => {
    ...
    const priceTable = lang === 'ru' ? PRICE_TABLE_RU : PRICE_TABLE_EN;
    const breadcrumbs = getBreadcrumbs(`/services/${String(serviceKey)}`, lang);
    ...
    return (
        <div className="min-h-screen bg-black pt-24 pb-20 px-4 md:px-12 flex flex-col lg:flex-row gap-12 relative overflow-hidden">
            ...
            <div className="flex-1 min-w-0 z-10 animate-float-in">
                ...
                {/* Price Table for GEO/SEO - Scannability */}
                <div className="mb-20 bg-zinc-950 border border-white/10 p-8 overflow-x-auto">
                    <h2 className="text-xl font-mono text-zinc-500 uppercase tracking-widest mb-6 flex items-center gap-3">
                        <BadgeCheck size={18} className="text-neon-acid" /> {lang === 'ru' ? 'Стоимость услуг CODEXAI' : 'CODEXAI Pricing'}
                    </h2>
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-white/10">
                                <th className="py-4 px-2 font-mono text-xs uppercase tracking-widest text-zinc-500">{lang === 'ru' ? 'Услуга' : 'Service'}</th>
                                <th className="py-4 px-2 font-mono text-xs uppercase tracking-widest text-zinc-500">{lang === 'ru' ? 'Цена' : 'Price'}</th>
                                <th className="py-4 px-2 font-mono text-xs uppercase tracking-widest text-zinc-500 hidden sm:table-cell">{lang === 'ru' ? 'Сроки' : 'Timeline'}</th>
                                <th className="py-4 px-2 font-mono text-xs uppercase tracking-widest text-zinc-500 hidden md:table-cell">{lang === 'ru' ? 'Гарантия' : 'Warranty'}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {priceTable.map((row, idx) => (
                                <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    <td className="py-4 px-2 text-white font-serif">{row.service}</td>
                                    <td className="py-4 px-2 text-neon-acid font-bold font-mono">{row.price}</td>
                                    <td className="py-4 px-2 text-zinc-400 hidden sm:table-cell">
                                        <span className="flex items-center gap-2">
                                            <Clock size={14} className="text-zinc-500" />
                                            {row.time}
                                        </span>
                                    </td>
                                    <td className="py-4 px-2 text-zinc-400 hidden md:table-cell">
                                        <span className="flex items-center gap-2">
                    ...
                    <p className="mt-4 text-xs text-zinc-500 font-mono">
                        {lang === 'ru'
                            ? '* Точная стоимость рассчитывается индивидуально после анализа задачи. Все цены и сроки фиксируются в договоре.'
                            : '* Exact price is calculated individually after task analysis. All prices and timelines are fixed in the contract.'
                        }
                    </p>
                </div>
